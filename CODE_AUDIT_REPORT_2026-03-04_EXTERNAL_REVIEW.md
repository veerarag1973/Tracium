...
Repository: AgentOBS (`agentobs` package)  
...
2. Ran lint/security-oriented static rules:
   - `d:/Sriram/AgentOBS/.venv/Scripts/python.exe -m ruff check agentobs tests`
...
- Immutability of event payload is now enforced via read-only view: [agentobs/event.py](agentobs/event.py#L370).
- Signature/validator format alignment appears correct (`sha256:` and `hmac-sha256:`): [agentobs/validate.py](agentobs/validate.py#L77-L78), [agentobs/signing.py](agentobs/signing.py#L141-L162).
- Governance strict unknown event-type enforcement exists: [agentobs/governance.py](agentobs/governance.py#L150-L157).
- OTLP batch chunking by configured batch size exists: [agentobs/export/otlp.py](agentobs/export/otlp.py#L524-L528).
...
1. Silent failure path in core dispatch (observability can fail with no signal)
   - Evidence: [agentobs/_stream.py](agentobs/_stream.py#L220-L257).
...
2. Emission failures swallowed in all span/run/step context managers
   - Evidence: [agentobs/_span.py](agentobs/_span.py#L327-L332), [agentobs/_span.py](agentobs/_span.py#L478-L481), [agentobs/_span.py](agentobs/_span.py#L612-L615).
...
3. Compliance package appears incomplete in runtime distribution path
   - Evidence: [tests/test_compliance.py](tests/test_compliance.py#L17) (`importorskip` because compliance submodules not yet ported), and `agentobs/compliance` currently contains only [agentobs/compliance/__init__.py](agentobs/compliance/__init__.py).
...
4. Egress URL validation is minimal; no SSRF/misrouting guardrails
   - Evidence: [agentobs/export/webhook.py](agentobs/export/webhook.py#L41-L48), [agentobs/export/otlp.py](agentobs/export/otlp.py#L44-L51), [agentobs/export/grafana.py](agentobs/export/grafana.py#L57-L63), [agentobs/export/datadog.py](agentobs/export/datadog.py#L110-L116).
...
5. Timestamp parsing fallback may silently rewrite time to now
   - Evidence: [agentobs/export/datadog.py](agentobs/export/datadog.py#L146-L149), [agentobs/export/grafana.py](agentobs/export/grafana.py#L193-L194).
...
6. In-memory ingestion constructors can be memory-heavy for large streams
   - Evidence: [agentobs/stream.py](agentobs/stream.py#L140-L154), [agentobs/stream.py](agentobs/stream.py#L201-L214), [agentobs/stream.py](agentobs/stream.py#L318-L335).
...
8. Legacy naming and encoding drift in docs/module text
   - Evidence: mixed references to `llm-toolkit-schema` and mojibake artifacts in [agentobs/__init__.py](agentobs/__init__.py#L1-L113) and CLI module docstring in [agentobs/_cli.py](agentobs/_cli.py#L1-L47).
   - Issue: Product naming inconsistency can confuse integrators and auditors.
   - Recommendation:
     - Normalize naming to AgentOBS/agentobs across code/docs and clean unicode artifacts.

## Hallucination Controls Review

### Current State

- The SDK provides strong telemetry primitives to record guard/fence events and outcomes (e.g., [agentobs/namespaces/guard.py](agentobs/namespaces/guard.py), [agentobs/namespaces/fence.py](agentobs/namespaces/fence.py)).
...
3. `ruff check agentobs tests` clean in CI (or rule set adjusted to realistic enforced baseline).
