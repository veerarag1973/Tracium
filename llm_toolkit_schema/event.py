...
* **Typed validation** — every validation failure is a
  :class:`~llm_toolkit_schema.exceptions.SchemaValidationError` with the field name,
...
* :class:`~llm_toolkit_schema.types.EventType` values serialised as their string value.
...
from llm_toolkit_schema.types import EventType, _EVENT_TYPE_RE  # noqa: PLC2701
from llm_toolkit_schema.ulid import generate as _generate_ulid
...
class Event:
    ...
    * ``event_type``     — namespaced string or :class:`~llm_toolkit_schema.types.EventType`
