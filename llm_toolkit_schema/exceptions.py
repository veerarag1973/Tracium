...
class SchemaValidationError(LLMSchemaError):
    """Raised when an :class:`~llm_toolkit_schema.event.Event` fails validation.
...
class SerializationError(LLMSchemaError):
    """Raised when an :class:`~llm_toolkit_schema.event.Event` cannot be serialized.
...
class VerificationError(LLMSchemaError):
    """Raised by :func:`~llm_toolkit_schema.signing.assert_verified` if an event fails
