"""llm_toolkit_schema.validate — JSON Schema validation for Event envelopes.

This module validates :class:`~llm_toolkit_schema.event.Event` instances against the
...
Public API
----------
* :func:`validate_event` — validate an :class:`~llm_toolkit_schema.event.Event`
  against the v1.0 envelope schema.
* :exc:`~llm_toolkit_schema.exceptions.SchemaValidationError` — raised on validation
  failure (re-exported from :mod:`llm_toolkit_schema.exceptions`).
"""
...
def _stdlib_validate(doc: Dict[str, Any]) -> None:
    ...
    JSON Schema spec.  Raises :exc:`~llm_toolkit_schema.exceptions.SchemaValidationError`
...
def validate_event(event: Event) -> None:
    ...
    ----------
    event:
        The :class:`~llm_toolkit_schema.event.Event` instance to validate.
