# AgentOBS SDK — Phased Implementation Plan
...
|---|---|---|---|
| 0 | **Package Rename** | Rename `llm_toolkit_schema` → `agentobs`; update all references | `agentobs/` importable, backward compat alias |
| 1 | **Configuration Layer** | `configure()` API, env var support, global config singleton | `agentobs.configure()` |
| 2 | **Core Tracer + Span** | `tracer.span()`, `SpanContextManager`, auto-population | `from agentobs import tracer` works end-to-end |
...
| 6 | **OpenAI Integration** | Auto-extract tokens + model from OpenAI response | `from agentobs.integrations import openai` |
...
| 9 | **Framework Integrations** | FastAPI, LangChain, LangGraph, CrewAI | `agentobs.integrations.*` modules |
| 10 | **CLI Tooling** | `agentobs check-compat`, `validate`, `audit-chain` | `agentobs` command available after install |
...
**Goal:** Rename the package from `llm_toolkit_schema` to `agentobs` with a backward-compatibility shim.

### Tasks

1. **Rename the package directory**
   - `llm_toolkit_schema/` → `agentobs/`
   - Update all `from llm_toolkit_schema import` → `from agentobs import` inside the package

2. **Update `pyproject.toml`**
   - `name = "agentobs"`
   - `version = "0.1.0"`
   - Update `[project.entry-points]` for CLI
   - Update package discovery: `packages = ["agentobs", "agentobs.*"]`
...
3. **Add backward-compatibility shim**
   ...
   import warnings
   warnings.warn(
       "llm_toolkit_schema is deprecated; use agentobs instead.",
       DeprecationWarning, stacklevel=2
   )
   from agentobs import *  # noqa: F401, F403
   from agentobs import __all__  # noqa: F401
   ```
...
- `import agentobs` succeeds
- `import llm_toolkit_schema` emits `DeprecationWarning` but does not raise
- All 42 namespace payload classes reachable via `agentobs.*`
- `agentobs.__version__ == "0.1.0"`
...
### Update `agentobs/__init__.py`

```python
from agentobs.config import configure
```

### Acceptance criteria

- `configure(exporter="jsonl", service_name="my-svc")` mutates the singleton
- `AGENTOBS_EXPORTER=console python -c "from agentobs import tracer"` picks up env var
...
On every span, AgentOBS MUST auto-assign:
...
```python
from agentobs._tracer import tracer
```
...
### New file: `agentobs/exporters/jsonl.py`

- Wraps `JSONLExporter` from `agentobs.export`
- Default output file: `agentobs_events.jsonl`
...
### New file: `agentobs/exporters/console.py`
...
### New file: `agentobs/integrations/openai.py`
...
| OpenAI response field | AgentOBS field |
|---|---|
...
```
agentobs/exporters/
    otlp.py          # OTLP gRPC/HTTP (requires agentobs[otlp])
    webhook.py       # HTTP POST with retry logic
    datadog.py       # Datadog log intake API (requires agentobs[datadog])
    grafana_loki.py  # Loki push API (requires agentobs[grafana-loki])
```
...
agentobs/integrations/
    ...
    otel.py          # OTel bridge: forwards OTel spans to AgentOBS
```

### FastAPI

```python
from agentobs.integrations.fastapi import AgentOBSMiddleware

app.add_middleware(AgentOBSMiddleware)
...
```python
from agentobs.integrations.langchain import AgentOBSCallbackHandler

chain = MyChain(callbacks=[AgentOBSCallbackHandler()])
...
**Goal:** `agentobs` command available after `pip install agentobs`.
...
```bash
agentobs validate agentobs_events.jsonl   # exits 0
agentobs stats agentobs_events.jsonl      # prints summary table
```
...
- `agentobs audit-chain` verifies the chain
...
- Coverage target: ≥90% line coverage on `agentobs/` core
- Performance test: emit 10,000 spans, verify <5ms overhead per span

### Documentation

- Full API reference (Sphinx, hosted on `agentobs.dev`)
...
- Migration guide from `llm_toolkit_schema` → `agentobs`
...
agentobs/
    ...
    config.py              # AgentOBSConfig + configure()
...
*End of AgentOBS SDK Implementation Plan*
