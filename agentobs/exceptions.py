...
All exceptions raised by agentobs inherit from :class:`LLMSchemaError` so
...
class SchemaValidationError(LLMSchemaError):
    """Raised when an :class:`~agentobs.event.Event` fails validation.
...
class SerializationError(LLMSchemaError):
    """Raised when an :class:`~agentobs.event.Event` cannot be serialized.
...
class VerificationError(LLMSchemaError):
    ...
    Raised by :func:`~agentobs.signing.assert_verified` on verification failure.
