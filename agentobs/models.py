...
This module provides Pydantic v2 models that mirror the :class:`~agentobs.event.Event`
...
Design goals
------------
* All field validation is equivalent to :meth:`~agentobs.event.Event.validate`,
...
class TagsModel(BaseModel):
    ...
    def from_tags(cls, tags: Tags) -> TagsModel:
        """Construct from a :class:`~agentobs.event.Tags` instance.

        Args:
            tags: A :class:`~agentobs.event.Tags` instance.
    ...
    def to_tags(self) -> Tags:
        """Convert back to a :class:`~agentobs.event.Tags` instance.

        Returns:
            A new :class:`~agentobs.event.Tags` with the same key-value pairs.
        """
...
class EventModel(BaseModel):
    ...
    :meth:`~agentobs.event.Event.validate`, so ``EventModel.from_event(event)``
    succeeds for any event that passes :meth:`~agentobs.event.Event.validate`.
    ...
    signature: str | None = Field(
        default=None,
        description="HMAC-SHA256 audit chain signature (set by agentobs.signing).",
    )
    ...
    def from_event(cls, event: Event) -> EventModel:
        """Construct an :class:`EventModel` from an :class:`~agentobs.event.Event`.

        Args:
            event: A validated or unvalidated :class:`~agentobs.event.Event`.
