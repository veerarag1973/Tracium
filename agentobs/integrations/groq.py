"""agentobs.integrations.groq — Auto-instrumentation for the Groq Python SDK.
...
* :class:`~agentobs.namespaces.trace.ModelInfo` (provider = ``groq``, name
  from response)
...
Usage::
    ...
    from agentobs.integrations import groq as groq_integration
    ...
    with agentobs.span("groq-chat", model="llama3-70b-8192") as span:
...
def patch() -> None:
    ...
    groq_mod._agentobs_patched = True  # type: ignore[attr-defined]
...
def unpatch() -> None:
    ...
    try:  # noqa: SIM105
        del groq_mod._agentobs_patched  # type: ignore[attr-defined]
    except AttributeError:  # pragma: no cover
        pass


def is_patched() -> bool:
    """Return ``True`` if the Groq client has been patched by agentobs.
...
def _require_groq() -> Any:  # noqa: ANN401
    ...
    try:
        ...
        raise ImportError(
            "The 'groq' package is required for agentobs Groq integration.\n"
