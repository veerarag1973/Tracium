...
Design goals
------------
* All field validation is equivalent to :meth:`~tracium.event.Event.validate`,
...
from tracium.types import EVENT_TYPE_PATTERN
from tracium.ulid import validate as _validate_ulid

__all__ = ["EventModel", "TagsModel"]

# ---------------------------------------------------------------------------
# Validation patterns (must stay in sync with tracium/event.py)
# ---------------------------------------------------------------------------
...
class TagsModel(BaseModel):
    ...
    def from_tags(cls, tags: Tags) -> TagsModel:
        ...
        Args:
            tags: A :class:`~tracium.event.Tags` instance.
    ...
    def to_tags(self) -> Tags:
        """Convert back to a :class:`~tracium.event.Tags` instance.

        Returns:
            A new :class:`~tracium.event.Tags` with the same key-value pairs.
        """
...
class EventModel(BaseModel):
    ...
    :meth:`~tracium.event.Event.validate`, so ``EventModel.from_event(event)``
    succeeds for any event that passes :meth:`~tracium.event.Event.validate`.
    ...
    def from_event(cls, event: Event) -> EventModel:
        ...
        Args:
            event: A validated or unvalidated :class:`~tracium.event.Event`.
