"""tracium.integrations.openai — Auto-instrumentation for the OpenAI Python SDK.
...
* :class:`~tracium.namespaces.trace.TokenUsage` (input / output / cached /
  reasoning token counts)
* :class:`~tracium.namespaces.trace.ModelInfo` (provider = ``openai``, name
  from response)
* :class:`~tracium.namespaces.trace.CostBreakdown` (computed from the static
  pricing table in :mod:`tracium.integrations._pricing`)
...
Usage::
    ...
    with tracium.span("chat", model="gpt-4o") as span:
...
def patch() -> None:
    ...
    openai_mod._tracium_patched = True  # type: ignore[attr-defined]
...
def is_patched() -> bool:
    """Return ``True`` if the OpenAI client has been patched by tracium.
...
def _compute_cost(
    ...
    Returns:
        A :class:`~tracium.namespaces.trace.CostBreakdown` instance.
    """
