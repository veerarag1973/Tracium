"""tracium.integrations.anthropic — Auto-instrumentation for the Anthropic Python SDK.
...
* :class:`~tracium.namespaces.trace.ModelInfo` (provider = ``anthropic``, name
  from response)
...
Usage::
    ...
    from tracium.integrations import anthropic as anthropic_integration
    ...
    with tracium.span("claude-chat", model="claude-3-5-sonnet-20241022") as span:
...
def patch() -> None:
    ...
    try:
        ...
        Messages._tracium_orig_create = _orig_sync  # type: ignore[attr-defined]
    ...
    try:
        ...
        AsyncMessages._tracium_orig_create = _orig_async  # type: ignore[attr-defined]
    except (ImportError, AttributeError):  # pragma: no cover
        pass

    anthropic_mod._tracium_patched = True  # type: ignore[attr-defined]
...
def is_patched() -> bool:
    """Return ``True`` if the Anthropic client has been patched by tracium.
