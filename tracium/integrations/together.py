"""tracium.integrations.together — Auto-instrumentation for the Together AI Python SDK.
...
* :class:`~tracium.namespaces.trace.ModelInfo` (provider = ``together_ai``,
...
is retained as :attr:`~tracium.namespaces.trace.ModelInfo.name` when the
normalized name is not found in the pricing table, to preserve observability
accuracy.

Usage::

    from tracium.integrations import together as together_integration
    ...
    with tracium.span("together-chat") as span:
...
def patch() -> None:
    ...
    together_mod._tracium_patched = True  # type: ignore[attr-defined]
...
def is_patched() -> bool:
    """Return ``True`` if the Together AI client has been patched by tracium.
...
def normalize_response(
    ...
    +--------------------------------------------+---------------------------+
    | Together AI field                          | Tracium field             |
    +============================================+===========================+
