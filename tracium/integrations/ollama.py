"""tracium.integrations.ollama — Auto-instrumentation for the Ollama Python SDK.
...
* :class:`~tracium.namespaces.trace.TokenUsage` (prompt / eval token counts
  mapped to input / output)
* :class:`~tracium.namespaces.trace.ModelInfo` (provider = ``ollama``, name
  from response)
* :class:`~tracium.namespaces.trace.CostBreakdown` (always zero — Ollama is a
  local runtime with no per-token billing)

Usage::

    from tracium.integrations import ollama as ollama_integration
    ...
    with tracium.span("ollama-chat", model="llama3") as span:
        resp = ollama.chat(
            model="llama3",
...
.. note::
   ...
   :meth:`~tracium.namespaces.trace.CostBreakdown.zero` for the cost field.
"""
...
def patch() -> None:
    ...
    if _orig_chat is not None:
        ...
        ollama_mod._tracium_orig_chat = _orig_chat  # type: ignore[attr-defined]
    ...
    try:
        ...
        Client._tracium_orig_chat = _orig_client_chat  # type: ignore[attr-defined]
    ...
    try:
        ...
        AsyncClient._tracium_orig_chat = _orig_async_chat  # type: ignore[attr-defined]
    except (ImportError, AttributeError):  # pragma: no cover
        pass

    ollama_mod._tracium_patched = True  # type: ignore[attr-defined]
...
def unpatch() -> None:
    ...
    orig_chat = getattr(ollama_mod, "_tracium_orig_chat", None)
...
def is_patched() -> bool:
    """Return ``True`` if Ollama has been patched by tracium.
