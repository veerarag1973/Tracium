...
* The bridge implements the :class:`~tracium.stream.Exporter`
  protocol so it is a drop-in replacement for
  :class:`~tracium.export.otlp.OTLPExporter` in any
  :class:`~tracium.stream.EventStream` pipeline.
...
class OTelBridgeExporter:
    ...
    Converts each :class:`~tracium.event.Event` into a completed
    ...
    Implements the :class:`~tracium.stream.Exporter` protocol —
