...
*   Signing failures always raise :exc:`~tracium.exceptions.SigningError`
    — never silently pass.
...
::
    ...
    from tracium.signing import sign, verify, verify_chain, AuditStream
...
def sign(
    ...
    Returns:
        A new :class:`~tracium.event.Event` with ``checksum``, ``signature``,
...
class AuditStream:
    ...
    def rotate_key(
        ...
        Returns:
            The signed key-rotation :class:`~tracium.event.Event`.
