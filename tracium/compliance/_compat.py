"""tracium.compliance._compat — RFC-0001 compatibility checks.

Verifies that a batch of :class:`~tracium.event.Event` objects conform to the
...
def _check_event(event: object) -> list[CompatibilityViolation]:  # type: ignore[type-arg]
    ...
    """
    from tracium.event import _SOURCE_PATTERN  # noqa: PLC0415
    from tracium.types import (  # noqa: PLC0415
        EventTypeError,
        is_registered,
        validate_custom,
    )
    from tracium.ulid import validate as _validate_ulid  # noqa: PLC0415
...
def test_compatibility(
    ...
    Args:
        events: Sequence of :class:`~tracium.event.Event` objects to inspect.
