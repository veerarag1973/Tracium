...
* **Typed validation** — every validation failure is a
  :class:`~tracium.exceptions.SchemaValidationError` with the field name,
...
* :class:`~tracium.types.EventType` values serialised as their string value.
...
class Event:
    ...
    * ``event_type``     — namespaced string or :class:`~tracium.types.EventType`
